  ...
  return (
    <div className="overflow-x-auto">
      {patterns.map((pattern, index) => (
        <table key={index} className="mx-auto my-8 table w-full table-fixed">
          <thead>
            <tr>
              <th className="text-center text-lg font-bold" colSpan={3}>
                {pattern.pattern}
              </th>
            </tr>
            <tr>
          ...
          <tbody>
            {pattern.traits.map((trait, traitIndex) => (
              <tr key={traitIndex}>
                ...
                <td className="whitespace-normal break-words">{trait.trait}</td>
                <td className="whitespace-normal break-words">
                  {trait.frequency}
                </td>
              </tr>
